...
class Atom:
    ...
    def squared_distance(self, other):
        # Utilisation de la distance au carré pour optimiser le temps de calcul (évite math.sqrt)
...
class HydrogenBond:
    ...
    def is_valid(self, cutoff=3.5):
        # On met le cutoff au carré pour le comparer à la squared_distance
...
def infer_base_pairs(hbonds):
    ...
    for (res1, res2), count in sorted(pair_counts.items(), key=lambda x: -x[1]):
        ...
        base_pairs.append(BasePair(res1[1], res2[1], res1[2], res2[2]))
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        print("Usage:")
        print(f" -> {sys.argv[0]} your_pdb_file.pdb")
        sys.exit(1)
